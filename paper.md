...
Across 51,445 IRS county pairs (2019--2020 tax year), Spearman rank correlation improves from $\rho = 0.041$ (population only) to $\rho = 0.052$ (adding geography) to $\rho = 0.165$ (adding data similarity, 95% CI: 0.155--0.173, bootstrap $n = 1000$). The $+0.112$ improvement from Model B to C ($p < 0.001$) demonstrates that the normalized socioeconomic distance captures county affinity beyond what population and geography predict. The absolute $\rho = 0.165$ is intentionally modest --- IRS migration is driven primarily by economic opportunity, not socioeconomic similarity, so a weak but significant improvement is the appropriate expectation. The validation establishes construct validity of the distance measure, not predictive validity for migration. Of the 51,445 pairs, approximately 24% appear in the pre-computed top-10,000 gravity link cache and receive non-zero predicted force; the remaining 76% receive zero. The Spearman $\rho$ therefore reflects both ranking accuracy within linked pairs and binary discrimination between linked and unlinked pairs.
