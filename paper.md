...
Here $h_{ij}$ is the Haversine geographic distance in miles, $\mathbf{v}_i$ and $\mathbf{v}_j$ are min-max normalized vectors across $k = 20$ socioeconomic datasets, $D_{\max}$ is the empirical maximum Euclidean distance across all county pairs, and the floor of 0.01 prevents numerical instability for adjacent identical-profile counties. With $\beta = 0.150$, population product explains 96.2% of raw force variance --- large counties attract strongly regardless of socioeconomic profile. The peer discovery feature uses data-space Euclidean distance directly (population-independent) to rank similar counties. The voter turnout rate variable uses total county population as denominator rather than citizen voting-age population (CVAP), introducing systematic underestimation in counties with large youth or non-citizen populations. Air quality data (EPA AQS) covers approximately 31% of counties; the remaining 69% receive midpoint imputation (sensitivity analysis: $r = 0.991$ between distance matrices with and without air quality, indicating negligible impact).

The gravity model, originally formulated for international trade flows [@tinbergen1962shaping], provides the theoretical foundation for this distance decay formulation. The parameter $\beta = 0.150$ is calibrated via OLS log-linear regression on 249,922 randomly sampled county pairs (seed = 42). A two-pass procedure first estimates $\beta_{geo} = 0.054$ ($R^2 = 0.035$) using geographic distance only, then $\beta_{combined} = 0.150$ ($R^2 = 0.299$) using the multiplicative combined distance. Pass 2 calibration contains a mechanical relationship: data_dissim appears in both the combined distance predictor and the similarity outcome. Cross-validation (80/20 split, seed = 42) shows $\beta_{cv} = 0.150$ vs $\beta_{op} = 0.150$ (difference = 0.0003) and $R^2_{holdout} = 0.300$ vs $R^2_{full} = 0.299$. The holdout $R^2$ marginally exceeds the full-sample $R^2$ (difference = 0.0025), providing strong evidence that the mechanical component in Pass 2 does not inflate the model fit. Combined with the out-of-sample IRS migration validation ($\rho = 0.165$, 95% CI: 0.155--0.173), the circularity does not distort the calibrated $\beta$ value, though the Pass 2 $R^2$ should be interpreted as an upper bound on true explanatory power rather than a conventional goodness-of-fit statistic.

# Validation

The model was validated against IRS Statistics of Income county-to-county migration flows (2019--2020, 51,445 pairs) [@irs_migration]. Spearman rank correlation improves from $\rho = 0.041$ (population only) to $\rho = 0.053$ (adding geography) to $\rho = 0.165$ (95% CI: 0.155--0.173, bootstrap $n=1000$) when adding data similarity, a $+0.112$ improvement beyond population and geography. Predicted force values are zero for pairs outside the top-10,000 pre-computed links; the validation therefore tests both ranking accuracy and link discrimination. Weighting robustness testing across three schemes (equal, domain-balanced, PCA-reduced) found maximum rho difference of 0.0009 and peer-level Jaccard similarity of 0.891 for domain-balanced weighting. External validation against FEMA National Risk Index [@fema_nri] confirmed disaster risk is largely independent of socioeconomic disadvantage ($r = 0.151$), establishing that the model captures a distinct welfare dimension. A benchmark comparison with @chetty2014land county-level poverty-mobility correlation ($r = -0.451$ vs. commuting zone $r \approx -0.60$) is consistent with known ecological aggregation effects. The Opportunity Atlas data [@opportunity_atlas] was excluded from the gravity model due to temporal mismatch (1978--2015 outcomes vs. 2022 conditions) but remains available for scatter plot exploration. DiscoSights separates two distinct operations: the gravity force calculation powers the terrain visualization and is validated against IRS migration flows, while peer discovery uses population-independent Euclidean distance in the 20-dimensional normalized data space, equivalent to $k$-nearest-neighbors (Jaccard = 1.000 between the two methods). For example, McDowell County WV finds Appalachian peers (Mingo WV 92%, Lake TN 91%) and Starr County TX finds Rio Grande Valley peers (Zapata TX 92%, Maverick TX 89%) --- structurally similar communities identified purely by data similarity regardless of population size.
