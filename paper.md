...
where $d_{ij} = \max(\text{geo\_norm}_{ij} \cdot \text{data\_dissim}_{ij},\; 0.01)$ combines normalized geographic distance (Haversine / 5,251 miles) and Euclidean data dissimilarity across 29 variables. $\beta$ is calibrated via two-pass OLS log-linear regression on 249,922 county pairs (seed = 42). Pass 1: $\beta_{geo} = 0.059$ ($R^2 = 0.054$, geographic only). Pass 2: $\beta = 0.139$ ($R^2 = 0.303$, combined distance). Cross-validation (80/20 holdout) confirms $\beta_{cv} = 0.139$, $R^2_{holdout} = 0.302$. Population product explains 96.7% of raw force variance; peer discovery uses population-independent Euclidean distance instead.
