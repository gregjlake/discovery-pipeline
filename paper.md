...
Here $h_{ij}$ is the Haversine geographic distance in miles, $\mathbf{v}_i$ and $\mathbf{v}_j$ are min-max normalized vectors across $k = 17$ socioeconomic datasets, $D_{\max}$ is the empirical maximum Euclidean distance across all county pairs, and the floor of 0.01 prevents numerical instability for adjacent identical-profile counties. With $\beta = 0.152$, population product explains 95.6% of raw force variance --- large counties attract strongly regardless of socioeconomic profile. The peer discovery feature uses data-space Euclidean distance directly (population-independent) to rank similar counties.

The gravity model, originally formulated for international trade flows [@tinbergen1962shaping], provides the theoretical foundation for this distance decay formulation. The parameter $\beta = 0.152$ is calibrated via OLS log-linear regression on 249,922 randomly sampled county pairs (seed = 42). A two-pass procedure first estimates $\beta_{geo} = 0.052$ ($R^2 = 0.038$) using geographic distance only, then $\beta_{combined} = 0.152$ ($R^2 = 0.306$) using the multiplicative combined distance. Pass 2 calibration contains a mechanical relationship: data_dissim appears in both the combined distance predictor and the similarity outcome. To test whether this inflates $\beta$, we estimated $\beta$ on an 80% training split and evaluated on a 20% holdout (seed = 42): $\beta_{cv} = 0.152$ vs $\beta_{op} = 0.152$ (difference = 0.0002), with $R^2_{holdout} = 0.309$ vs $R^2_{full} = 0.306$ (inflation = 0.0002). $\beta$ is therefore stable to the circularity. Independently, the IRS migration validation ($\rho = 0.164$, 95% CI: 0.155--0.173) provides external evidence of model validity free of any calibration circularity --- migration flows were never used in calibration. Together, these two tests establish that the circularity concern is negligible in practice.
