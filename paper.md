...
The gravity model, originally formulated for international trade flows [@tinbergen1962shaping], provides the theoretical foundation for this distance decay formulation. The parameter $\beta = 0.155$ is calibrated via OLS log-linear regression on 249,922 randomly sampled county pairs (seed = 42). A two-pass procedure first estimates $\beta_{geo} = 0.055$ ($R^2 = 0.038$) using geographic distance only, then $\beta_{combined} = 0.155$ ($R^2 = 0.313$) using the multiplicative combined distance. Model fit improved from $R^2 = 0.038$ to $R^2 = 0.313$, noting that Pass 2 $R^2$ includes a mechanical component from shared variables between the distance measure and dependent variable. Out-of-sample IRS migration validation ($\rho = 0.165$) provides the appropriate measure of predictive validity.
