...
authors:
  - name: Greg Lake
    orcid: 0000-0000-0000-0000  # TODO: Replace with real ORCID from orcid.org before submission
    affiliation: 1
affiliations:
...
Here $h_{ij}$ is the Haversine geographic distance in miles, $\mathbf{v}_i$ and $\mathbf{v}_j$ are min-max normalized vectors across $k = 17$ socioeconomic datasets, $D_{\max}$ is the empirical maximum Euclidean distance across all county pairs, and the floor of 0.01 prevents numerical instability for adjacent identical-profile counties. With $\beta = 0.155$, population product explains 95.6% of raw force variance --- large counties attract strongly regardless of socioeconomic profile. The peer discovery feature uses data-space Euclidean distance directly (population-independent) to rank similar counties.

The gravity model, originally formulated for international trade flows [@tinbergen1962shaping], provides the theoretical foundation for this distance decay formulation. The parameter $\beta = 0.155$ is calibrated via OLS log-linear regression on 249,922 randomly sampled county pairs (seed = 42). A two-pass procedure first estimates $\beta_{geo} = 0.055$ ($R^2 = 0.038$) using geographic distance only, then $\beta_{combined} = 0.155$ ($R^2 = 0.313$) using the multiplicative combined distance. To address the mechanical relationship between predictor and outcome in Pass 2, $\beta$ was estimated on an 80% training split and evaluated on a 20% holdout (seed = 42). $\beta_{cv} = 0.155$ (vs $\beta_{op} = 0.155$, difference = 0.0002) and $R^2_{holdout} = 0.313$ (vs $R^2_{full} = 0.313$, inflation = 0.0002). The circularity concern is negligible --- $\beta$ and $R^2$ are stable under cross-validation. Researchers can explore $\beta$ sensitivity interactively via the tool's $\beta$ selector in the Map view.

# Validation

The model was validated against IRS Statistics of Income county-to-county migration flows (2019--2020, 51,445 pairs) [@irs_migration]. Spearman rank correlation improves from $\rho = 0.041$ (population only) to $\rho = 0.053$ (adding geography) to $\rho = 0.165$ (95% CI: 0.156--0.174, bootstrap $n=1000$) when adding data similarity, a $+0.113$ improvement beyond population and geography. Predicted force values are zero for pairs outside the top-10,000 pre-computed links; the validation therefore tests both ranking accuracy and link discrimination. Weighting robustness testing across three schemes (equal, domain-balanced, PCA-reduced) found maximum rho difference of 0.0009 and peer-level Jaccard similarity of 0.891 for domain-balanced weighting. External validation against FEMA National Risk Index [@fema_nri] confirmed disaster risk is largely independent of socioeconomic disadvantage ($r = 0.151$), establishing that the model captures a distinct welfare dimension. A benchmark comparison with @chetty2014land county-level poverty-mobility correlation ($r = -0.451$ vs. commuting zone $r \approx -0.60$) is consistent with known ecological aggregation effects. The Opportunity Atlas data [@opportunity_atlas] was excluded from the gravity model due to temporal mismatch (1978--2015 outcomes vs. 2022 conditions) but remains available for scatter plot exploration.
